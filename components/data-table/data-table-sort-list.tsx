  ...
  return (
    ...
    >
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger render={<Button variant="outline" className="font-normal" onKeyDown={onTriggerKeyDown} disabled={disabled} />}><ArrowDownUp className="text-muted-foreground" />Ordenar
                          {sorting.length > 0 && (
                            <Badge
                              variant="secondary"
        ...
        >
          <div className="flex flex-col gap-1">
            <h4 id={labelId} className="font-medium leading-none">
              {sorting.length > 0 ? "Ordenar por" : "Nenhuma ordenação aplicada"}
            </h4>
            <p
              id={descriptionId}
              className={cn(
            ...
            >
              {sorting.length > 0
                ? "Ajuste a ordem para organizar suas transações."
                : "Adicione uma ordenação para organizar suas transações."}
            </p>
          </div>
          {sorting.length > 0 && (
            <SortableContent>
