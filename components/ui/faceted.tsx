...
function Faceted<Multiple extends boolean = false>(
  ...
  type OpenChangeDetails = Parameters<
    NonNullable<FacetedProps<Multiple>["onOpenChange"]>
  >[1];

  const onOpenChange = React.useCallback(
    (newOpen: boolean, eventDetails?: OpenChangeDetails) => {
      if (!isControlled) {
        setUncontrolledOpen(newOpen);
      }
      onOpenChangeProp?.(newOpen, eventDetails as OpenChangeDetails);
    },
...
function FacetedContent(props: React.ComponentProps<typeof PopoverContent>) {
  ...
  return (
    <PopoverContent
      ...
      align="start"
      className={cn(
        "w-50origin-(--radix-popover-content-transform-origin) p-0",
        className,
      )}
    >
